...
function arrangeDominoes(playersDominoes, currentPlayerDominoes, currentTrain, usedDominoes) {
  ...
  for (let i = 0; i < currentPlayerDominoes.length; i++) {
      ...
      if (usedDominoes.some(usedDomino => (usedDomino.a === domino.a && usedDomino.b === domino.b) || (usedDomino.a === domino.b && usedDomino.b === domino.a))) {
          continue;
      }
      if (currentTrain.length === 0 || currentTrain[currentTrain.length - 1].b === domino.a) {
          let newTrain = arrangeDominoes(playersDominoes, currentPlayerDominoes, currentTrain.concat(domino), usedDominoes.concat(domino));
      ...
      if (currentTrain.length === 0 || currentTrain[currentTrain.length - 1].b === domino.b) {
          let newTrain = arrangeDominoes(playersDominoes, currentPlayerDominoes, currentTrain.concat({a: domino.b, b: domino.a}), usedDominoes.concat(domino));
          if (newTrain.length > longestTrain.length) {
              longestTrain = newTrain;
          }
      }
...
function printResponse(playerTrains, playersDominoes) {
  ...
  for (let player in playerTrains) {
      ...
      let remainingDominoes = playersDominoes[player].filter(domino => !train.includes(domino));
      response[player] = {
